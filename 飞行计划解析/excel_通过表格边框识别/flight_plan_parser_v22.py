...
class FlightPlanParser:
    def __init__(self, filepath: str, sheet_name: str = None):
        ...
        self.border_regions = []
        self.metadata = {}
        # 行号配置（可能被 _find_boundaries 动态调整）
    ...
    def _build_time_map(self):
        ...
        for col in range(self.data_start_col - 1, 0, -1):
            h = self.ws.cell(row=self.hour_time_row, column=col).value
        ...
        for col in range(self.data_start_col + 1, self.data_end_col):
            h = self.ws.cell(row=self.hour_time_row, column=col).value
            m = self.ws.cell(row=self.min_time_row, column=col).value
            if h and isinstance(h, str) and ':' in h:
                current_hour = int(h.split(':')[0].strip())
    ...
    def _extract_metadata(self):
        ...
        if self.data_end_col:
            remarks_val = ws.cell(row=self.start_row, column=self.data_end_col).value
    ...
    def _scan_borders(self):
        self.border_regions = []
        for row_cells in self.ws.iter_rows(min_row=self.start_row):
