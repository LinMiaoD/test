...
def scan_bold_borders(filepath, sheet_name=None):
    ...
    if sheet_name:
        if sheet_name not in wb.sheetnames:
            print(f"警告: Sheet '{sheet_name}' 不存在，可用的 Sheet: {wb.sheetnames}")
            print(f"将使用第一个 Sheet: '{wb.sheetnames[0]}'")
            sheet_name = None
    sheets = [sheet_name] if sheet_name else [wb.sheetnames[0]]
    all_results = []
...
if __name__ == '__main__':
    ...
    # 把除第一个参数外的所有参数拼起来，兼容路径含空格的情况
    filepath = ' '.join(sys.argv[1:])
    # 如果用户在末尾加了 sheet 名，尝试分离（可选）
    # 默认只读第一个 sheet
