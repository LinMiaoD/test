    ...
    def _parse_sortie_from_region(self, region: BorderRegion):
        ...
        for search_row in (row, row - 1):
            ...
            if course:
                break
        if course is None:
            # 调试: 打印 row 和 row-1 的前几个有值单元格
            for dr in (row, row - 1):
                debug_vals = []
                for col in range(sc, min(sc + 10, ec + 2)):
                    v = self.ws.cell(row=dr, column=col).value
                    if v is not None:
                        debug_vals.append(f"col{col}={repr(v)}")
                print(f"  [调试] 边框行{row}({sc}-{ec}), "
                      f"搜索行{dr}: {debug_vals if debug_vals else '全空'}")
            course = "未识别"
        ...
        # [调试] 打印 ec+1, ec+2, ec+3 列在 row, row+1, row+2 的值
        print(f"  [调试] 边框行{row}({sc}-{ec}), 人员搜索区域:")
        for dr in (row, row + 1, row + 2):
            vals = []
            for dc in range(ec + 1, ec + 4):
                v = self.ws.cell(row=dr, column=dc).value
                if v is not None:
                    vals.append(f"col{dc}={repr(v)}")
                else:
                    vals.append(f"col{dc}=None")
            print(f"    行{dr}: {vals}")
